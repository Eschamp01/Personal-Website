...
Each activity heading should be in h4 tags, and be in the form '[Morning/Afternoon/Evening] - [activity name]'. \n"
    ...
    # Using the weather information obtained from a weather API, plan activities according to the weather.
    if dict['weather_string'] is not "":
        prompt+= f"Here is information about the weather for the trip: *** {dict['weather_string']} *** . Try to move indoor activities to days when \
it is expected to rain. If it is not expected to rain, or there are no good indoor activities, then no need to adjust the plan. "
